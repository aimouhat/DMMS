...
const TeamActions: React.FC = () => {
  ...
  const disciplines = Object.keys(actionsByDiscipline).sort();
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-grow w-full px-2 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8">
        <div className="max-w-full mx-auto">
          ...
          {/* Actions Table by Discipline */}
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gradient-to-r from-blue-900 to-blue-800">
                  <tr>
                    <th className="px-4 sm:px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">
                      Discipline
                    </th>
                    <th className="px-4 sm:px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">
                      Actions
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {disciplines.map((discipline, disciplineIndex) => (
                    <tr key={discipline} className={disciplineIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      <td className="px-4 sm:px-6 py-6 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            <div className="text-sm font-medium text-gray-900">{discipline}</div>
                            <div className="text-sm text-gray-500">
                              {actionsByDiscipline[discipline].length} open action{actionsByDiscipline[discipline].length !== 1 ? 's' : ''}
                            </div>
                          </div>
                        </div>
                      </td>
                              ...
                              <div className="flex items-start justify-between">
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center space-x-2 mb-2">
                                    ...
                                    <span className={`text-xs font-medium px-2 py-1 rounded-full ${getStatusTextColor(action.status)} bg-opacity-10`}>
                                      {action.status}
                                    </span>
                                    <span className="text-xs text-gray-500">#{action.id}</span>
                                  </div>
                                  <h4 className="text-sm font-medium text-gray-900 mb-2 group-hover:text-blue-600 transition-colors">
                                    {action.actionPlan}
                                  </h4>
                                    ...
                                    {action.criticality && (
                                      <div className="flex items-center space-x-1">
                                        ...
                                        <span className={`font-medium ${
                                          action.criticality === 'Critical' ? 'text-red-600' :
                                          action.criticality === 'High' ? 'text-orange-600' :
                                          action.criticality === 'Medium' ? 'text-yellow-600' :
                                          'text-green-600'
                                        }`}>
                                          {action.criticality}
                                        </span>
